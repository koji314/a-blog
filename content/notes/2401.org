...
#+LATEX_HEADER: }
* Preamble - Review
*Please review the notions of groups, rings, and fields by, e.g, looking at the Wikipedia entries.*
** Groups
** Rings
** Fields
...
And if \(f\) wasn't negative, then \([a,\alpha - \delta]\) would mean that \(\alpha - \delta \leq \alpha - \delta\), which is obviously a contradiction, as \(\alpha\) is the supremum among these numbers.
